...
Процедура ЗаполнитьРолиПользователяИБ(ПользовательИБ, СоставРолей)
	...
	КонецЕсли; 
// ++ УГМК Игитов М.А. 09.05.2010 17:08:33 
	Если НЕ ПользовательИБ.Роли.Содержит(Метаданные.Роли.УГМК_Пользователь)
		И НЕ ПользовательИБ.Роли.Содержит(Метаданные.Роли.ПолныеПрава) Тогда
		// Роль "УГМК_Пользователь" назначается всем
		ПользовательИБ.Роли.Добавить(Метаданные.Роли.УГМК_Пользователь);
	КонецЕсли; 
// -- УГМК Игитов М.А.  09.05.2010 17:08:35 
КонецПроцедуры //
...
//
Процедура ИнформироватьОбИзмененииНастроекПользователя(Знач НаборЗаписей) Экспорт
