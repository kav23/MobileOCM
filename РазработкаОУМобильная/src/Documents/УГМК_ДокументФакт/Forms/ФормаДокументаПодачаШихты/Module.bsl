...
&НаКлиенте
Процедура ОбработкаСканированияКлавиатура()
	Перем СтрокаШтрихкода1, ИнформацияБирки;
	...
	//КонецЕсли;
		...
		СтрокаШтрихкода1 = СтрЗаменить( СтрокаШтрихКод, Символы.ВК, "");
		СтрокаШтрихкода1 = СтрЗаменить( СтрокаШтрихкода1, Символы.ПС, "");
		ТекстПредупреждения = "";
		ТекстШапкиПредупреждения = "";
		Если ПрочитатьСтрокуЭтикетки( СтрокаШтрихкода1, ИнформацияБирки) тогда
...
Процедура ПрочитатьИзСтроки(Команда)
	...
	СтрокаШтрихКод = "";
	ЭтаФорма.ТекущийЭлемент = Элементы.КопироватьСтроку;
	ЭтаФорма.ТекущийЭлемент = Элементы.СтрокаШтрихКод;
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	УстановитьОтборМаркиСплава();
	Объект.Организация = Константы.Организация.Получить();
	
КонецПроцедуры

&НаСервере
Процедура УстановитьОтборМаркиСплава()
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	УГМК_МаркаСплаваПроизводство.Активная";
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
	Таблица = РезультатЗапроса.Выгрузить();
	Массив = Таблица.ВыгрузитьКолонку("Ссылка");
	Элементы.МаркаСплаваОбъект.СписокВыбора.Очистить();
	Элементы.МаркаСплаваОбъект.СписокВыбора.ЗагрузитьЗначения(Массив);
КонецПроцедуры
...
Процедура ОбновитьОтображениеИнформации()
	...
	ОтборВыбранные.Вставить("ТараИспользоваласьРанее", Истина);
	Этаформа.Элементы.СписокТарыВыбранные.ОтборСтрок = Новый ФиксированнаяСтруктура(ОтборВыбранные);
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Элементы.ГруппаВвестиКоличество.Видимость Тогда
		...
		МассивВидимыхГрупп.Добавить(Элементы.ГруппаОбщая);
	...
	ИначеЕсли Элементы.ГруппаСписокВыбранныхМатериалов.Видимость Тогда
		...
		УправлениеВидимостью(МассивВидимыхГрупп);	
		Возврат;
	ИначеЕсли Элементы.ГруппаОбщая.Видимость Тогда
		Отказ = Истина;
		СтандартнаяОбработка = Ложь;
		МассивВидимыхГрупп = Новый Массив;
		МассивВидимыхГрупп.Добавить(Элементы.ГруппаШапка);
...
Процедура УправлениеВидимостью(МассивВидимыхГрупп)
	...
	МассивВсехГрупп.Добавить(Элементы.ГруппаОбщая);
...
&НаКлиенте
Процедура ОтменаВыбранныеТовары(Команда)
	МассивВидимыхГрупп = Новый Массив;
	МассивВидимыхГрупп.Добавить(Элементы.ГруппаШапка);
 	УправлениеВидимостью(МассивВидимыхГрупп);	
	ОбновитьОтображениеИнформации();
