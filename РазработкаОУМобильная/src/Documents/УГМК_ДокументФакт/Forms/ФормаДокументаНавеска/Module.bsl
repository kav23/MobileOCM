...
&НаКлиенте
Процедура СканироватьКороб(Команда)
	СтартСканировать();	
КонецПроцедуры
		...
		Если ПрочитатьСтрокуЭтикетки( СтрокаШтрихкода1, ИнформацияБирки) тогда
			...
			Для каждого Строка из ИнформацияБирки.ДанныеБирки цикл
				...
				Если МассивСтрокПоОтбору.Количество() = 0 Тогда
					...
					Элементы.СписокТары.ТекущаяСтрока = ИдентификаторТекущейСтроки;
...
Процедура ОбработкаСканированияКлавиатура() Экспорт
	...
	РезультатШК = "";
		//Сообщить("0");
		//Сообщить("Текст1: " + Лев(Элементы.СтрокаШтрихКод.ТекстРедактирования,10));
	...
	Если  Элементы.СтрокаШтрихКод.ТекстРедактирования <> ""  ИЛИ СтрокаШтрихКод <> "" Тогда
		...
		//Сообщить("Текст2: " + Лев(Элементы.СтрокаШтрихКод.ТекстРедактирования,10));
		ОчиститьСтрокаШтрихКод();
		Элементы.СтрокаШтрихКод.ОбновитьТекстРедактирования();
		
		//Сообщить("Текст3: " + Лев(Элементы.СтрокаШтрихКод.ТекстРедактирования,10));
	...
	Попытка
		...
		ТекстПредупреждения = "";
		ТекстШапкиПредупреждения = "";
		//Сообщить("Текст31: " + Лев(Элементы.СтрокаШтрихКод.ТекстРедактирования,10));
		Если ПрочитатьСтрокуЭтикетки( СтрокаШтрихкода1, ИнформацияБирки) тогда
			//ТаблицаМатериалы.Очистить();
			Если ИнформацияБирки.ДанныеБирки.Количество() > 0 Тогда
				Сигнал();
				//сообщить("1");
		//Сообщить("Текст32: " + Лев(Элементы.СтрокаШтрихКод.ТекстРедактирования,10));
			Иначе	
				ТекстПредупреждения = "ШК не прочитан";
				ТекстШапкиПредупреждения = "Внимание!!!";
				ПараметрыПредупреждения = Новый Структура;
			...
			//СредстваМультимедиа.ЗакрытьСканированиеШтрихКодов();
			//сообщить("4");
		//Сообщить("Текст33: " + Лев(Элементы.СтрокаШтрихКод.ТекстРедактирования,10));
			
			Для каждого Строка из ИнформацияБирки.ДанныеБирки цикл
				//сообщить("5");
			//Сообщить("Текст34: " + Лев(Элементы.СтрокаШтрихКод.ТекстРедактирования,10));
			СтруктураОтбора = ПолучитьИменаКолонокВерсииОтборВерсия();
				ЗаполнитьЗначенияСвойств(СтруктураОтбора, Строка);
				МассивСтрокПоОтбору = объект.Материалы.НайтиСтроки(СтруктураОтбора);
				Если МассивСтрокПоОтбору.Количество() = 0 Тогда
		//Сообщить("Текст35: " + Лев(Элементы.СтрокаШтрихКод.ТекстРедактирования,10));
					НоваяСтрока = объект.Материалы.Добавить();
					ЗаполнитьЗначенияСвойств( НоваяСтрока, Строка);
					НоваяСтрока.Датаоперации = ТекущаяДата();
					НоваяСтрока.Представление = ПредставлениеСтрокиКлиентКратко(НоваяСтрока);
					...
					ТекстШапкиПредупреждения = "Добавлена";
					Модифицированность = Истина;
		//Сообщить("Текст36: " + Лев(Элементы.СтрокаШтрихКод.ТекстРедактирования,10));
					//сообщить("7");
					
					//КонецЕсли;
				Иначе
		//Сообщить("Текст37: " + Лев(Элементы.СтрокаШтрихКод.ТекстРедактирования,10));
					ИдентификаторТекущейСтроки = МассивСтрокПоОтбору[0].ПолучитьИдентификатор();
					Элементы.Материалы.ТекущаяСтрока = ИдентификаторТекущейСтроки;
					ТекстПредупреждения = ТекстПредупреждения + ?(ЗначениеЗаполнено(ТекстПредупреждения),"
					|","") + ПредставлениеСтрокиПредупреждение(МассивСтрокПоОтбору[0]);
					ТекстШапкиПредупреждения = "Строка есть";
		//Сообщить("Текст38: " + Лев(Элементы.СтрокаШтрихКод.ТекстРедактирования,10));
					//сообщить("8");
				КонецЕсли;
				//сообщить("9");
			...
			ЗаписатьПараметрыПредупреждения(ПараметрыПредупреждения);
			//сообщить("10");
		//Сообщить("Текст4: " + Лев(Элементы.СтрокаШтрихКод.ТекстРедактирования,10));
		КонецЕсли;
		Результат = "";
	Исключение
	...
	ОткрытьИнтерфейсПосле = Истина;
		//Сообщить("Текст5: " + Лев(Элементы.СтрокаШтрихКод.ТекстРедактирования,10));
	//Сообщить( "Закрытиие сканирования ");
	//Иначе
	//	Текст = "Ошибка: " + Сообщение;
		//сообщить("11");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	//ЗначениеВРеквизитФормы(Параметры.Объект,"Количество");
	//ЗначениеВРеквизитФормы(1,"Количество");
	...
	Элементы.СканироватьКороб.Видимость = СканированиеКомпонентой1С;
...
Процедура ОбновитьОтображениеИнформации()
	...
	Отбор.Вставить("ТараИспользоваласьРанее", Ложь);
	Этаформа.Элементы.СписокТары.ОтборСтрок = Новый ФиксированнаяСтруктура(Отбор);
КонецПроцедуры	
...
Процедура СохранитьДокументОбъект(ИмяТабличнойЧасти)
	...
	//		СтрокаМатериалы.Количество = СтрокаМатериалы.КоличествоВыбрали;
	//	КонецЦикла;	
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ЗаполнитьЗначенияСвойств(НоваяЗапись, НаборЗаписей.Отбор);
	НоваяЗапись.ID = ID;
	НоваяЗапись.Пользователь = ТекущийПользователь;
	НоваяЗапись.Настройка = "РабочийЦентр";
	НоваяЗапись.Значение = Новый ХранилищеЗначения(Объект.РабочийЦентр, Новый СжатиеДанных(9));
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Элементы.ГруппаВвестиКоличество.Видимость Тогда
		Отказ = Истина;
		СтандартнаяОбработка = Ложь;
		МассивВидимыхГрупп = Новый Массив;
		МассивВидимыхГрупп.Добавить(Элементы.ГруппаНавескаОбщая);
		УправлениеВидимостью(МассивВидимыхГрупп);	
		Возврат;
	ИначеЕсли Элементы.ГруппаСписокВыбранныхМатериалов.Видимость Тогда
		Отказ = Истина;
		СтандартнаяОбработка = Ложь;
		МассивВидимыхГрупп = Новый Массив;
		МассивВидимыхГрупп.Добавить(Элементы.ГруппаШапка);
		УправлениеВидимостью(МассивВидимыхГрупп);	
		Возврат;
	ИначеЕсли Элементы.ГруппаНавескаОбщая.Видимость Тогда
		Отказ = Истина;
		СтандартнаяОбработка = Ложь;
		МассивВидимыхГрупп = Новый Массив;
		МассивВидимыхГрупп.Добавить(Элементы.ГруппаШапка);
...
&НаКлиенте
Процедура ВзятьИзКороба(Команда)
	// Вставить содержимое обработчика.
	//Элементы.ГруппаВвестиКоличество.Видимость = Истина;
	//Элементы.ГруппаНавескаОбщая.Видимость = Ложь;
	ЗаполнитьДанныеСтроки(Элементы.СписокТары.ДанныеСтроки(Элементы.СписокТары.ТекущаяСтрока));
	МассивВидимыхГрупп = Новый Массив;
	МассивВидимыхГрупп.Добавить(Элементы.ГруппаВвестиКоличество);
 	УправлениеВидимостью(МассивВидимыхГрупп);	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьДанныеСтроки(Данные)
	// Вставить содержимое обработчика.
	ПоСвойствам = 		Данные.ПоСвойствам;
	Номенклатура = 		Данные.Номенклатура;
	МаркаСплава = 		Данные.МаркаСплава;
	ГруппаПродукции = 	Данные.ГруппаПродукции;
	Тара =		 		Данные.Тара;
	Количество = 		Данные.Количество;
	КоличествоТара = 	Данные.КоличествоТара;
	КоличествоВыбрали =	Данные.КоличествоВыбрали;
	КоличествоОсталось = КоличествоТара + Количество - КоличествоВыбрали;	
КонецПроцедуры


&НаКлиенте
Процедура ОтменитьВводКоличество(Команда)
	// Вставить содержимое обработчика.
	МассивВидимыхГрупп = Новый Массив;
	МассивВидимыхГрупп.Добавить(Элементы.ГруппаНавескаОбщая);
 	УправлениеВидимостью(МассивВидимыхГрупп);	
...
&НаКлиенте
Процедура СохранитьКоличество(Команда)
	ТекущаяСтрокаДокумента = объект.материалы.Получить(Элементы.СписокТары.ТекущаяСтрока);
	ТекущаяСтрокаДокумента.ДатаОперации=		ТекущаяДата();
	ТекущаяСтрокаДокумента.ПоСвойствам = 		ПоСвойствам;
	ТекущаяСтрокаДокумента.Номенклатура = 		Номенклатура;
	ТекущаяСтрокаДокумента.МаркаСплава = 		МаркаСплава;
	ТекущаяСтрокаДокумента.ГруппаПродукции = 	ГруппаПродукции;
	ТекущаяСтрокаДокумента.Тара =		 		Тара;
	ТекущаяСтрокаДокумента.КоличествоВыбрали = 	КоличествоВыбрали;
	ТекущаяСтрокаДокумента.КоличествоТара = 	КоличествоТара;
	Если ТекущаяСтрокаДокумента.КоличествоВыбрали > 0 Тогда
		ТекущаяСтрокаДокумента.ТараИспользоваласьРанее = Истина;
	КонецЕсли;		
	МассивВидимыхГрупп = Новый Массив;
	МассивВидимыхГрупп.Добавить(Элементы.ГруппаНавескаОбщая);
 	УправлениеВидимостью(МассивВидимыхГрупп);	
...
&НаКлиенте
Процедура КоличествоОсталосьПриИзменении(Элемент)
	// Вставить содержимое обработчика.
	КоличествоВыбрали = Количество - КоличествоОсталось;
КонецПроцедуры

&НаКлиенте
Процедура КоличествоВыбралиПриИзменении(Элемент)
	КоличествоОсталось = Количество - КоличествоВыбрали;
КонецПроцедуры

&НаКлиенте
Процедура КоличествоВыбралиОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, Параметры, СтандартнаяОбработка)
	КоличествоОсталось = Количество - КоличествоВыбрали;
КонецПроцедуры

&НаКлиенте
Процедура КоличествоОсталосьНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура ВвестиЧислоФорма(Команда)
	СтандартнаяОбработка = Ложь;
	...
	КоличествоВыбрали = Количество + КоличествоТара - КоличествоОсталось;
КонецПроцедуры

&НаКлиенте
Процедура ВвестиЧислоВыбрали(Команда)
	СтандартнаяОбработка = Ложь;
	ПараметрыВ = Новый Структура;
	ПараметрыВ.Вставить("Заголовок", "Взяли");
	Результат = ОткрытьФормуМодально("Обработка.УГМК_ВводЧисла.Форма.Форма",ПараметрыВ, ЭтаФорма);
	НовоеЗначение = Неопределено;
	Если Результат <> Неопределено тогда
		НовоеЗначение = Результат.Значение;
	КонецЕсли;
	КоличествоВыбрали = НовоеЗначение;
	КоличествоОсталось = Количество + КоличествоТара - КоличествоВыбрали;
КонецПроцедуры

&НаКлиенте
Процедура ВвестиЧислоОсталось(Команда)
	СтандартнаяОбработка = Ложь;
	ПараметрыВ = Новый Структура;
	ПараметрыВ.Вставить("Заголовок", "Осталось");
	Результат = ОткрытьФормуМодально("Обработка.УГМК_ВводЧисла.Форма.Форма",ПараметрыВ, ЭтаФорма);
	НовоеЗначение = Неопределено;
	Если Результат <> Неопределено тогда
		НовоеЗначение = Результат.Значение;
...
&НаКлиенте
Процедура ПерейтиКВыборуТары(Команда)
	//Элементы.ГруппаШапка.Видимость = Ложь;
	//Элементы.ГруппаНавескаОбщая.Видимость = Истина;
	МассивВидимыхГрупп = Новый Массив;
	МассивВидимыхГрупп.Добавить(Элементы.ГруппаНавескаОбщая);
 	УправлениеВидимостью(МассивВидимыхГрупп);	
КонецПроцедуры
...
&НаКлиенте
Процедура ВыбратьТаруИзСписка(Команда)
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура КоличествоОсталосьАвтоПодбор(Элемент, Текст, ДанныеВыбора, Параметры, Ожидание, СтандартнаяОбработка)
	ТекстВвода = Текст;
КонецПроцедуры
...
&НаКлиенте
Процедура ПерейтиКНавеске(Команда)
	МассивВидимыхГрупп = Новый Массив;
	МассивВидимыхГрупп.Добавить(Элементы.ГруппаНавескаОбщая);
 	УправлениеВидимостью(МассивВидимыхГрупп);
	ОбновитьОтображениеИнформации();
...
Процедура УправлениеВидимостью(МассивВидимыхГрупп)
	...
	МассивВсехГрупп.Добавить(Элементы.ГруппаДокумента);
	МассивВсехГрупп.Добавить(Элементы.ГруппаНавескаОбщая);
	МассивВсехГрупп.Добавить(Элементы.ГруппаВвестиКоличество);
	МассивВсехГрупп.Добавить(Элементы.ГруппаШапка);
	МассивВсехГрупп.Добавить(Элементы.ГруппаВыборТарыИзСписка); 
	МассивВсехГрупп.Добавить(Элементы.ГруппаВыбораПользователя); 
	МассивВсехГрупп.Добавить(Элементы.ГруппаСписокВыбранныхМатериалов); 
...
&НаКлиенте
Процедура ПосмотретьВыбранныеМатериалы(Команда)
	ОбновитьОтображениеИнформации();
	МассивВидимыхГрупп = Новый Массив;
	МассивВидимыхГрупп.Добавить(Элементы.ГруппаСписокВыбранныхМатериалов);
 	УправлениеВидимостью(МассивВидимыхГрупп);	
