...
Процедура ОК(Команда)
	...
	ПарметрыОповещения.Вставить("Тара", Тара);
	...
	ПарметрыОповещения.Вставить("МаркаСплава", МаркаСплава);
	ПарметрыОповещения.Вставить("НоваяСтрока", НоваяСтрока);
	ПарметрыОповещения.Вставить("КоличествоТара", КоличествоТара);
	...
	#КонецЕсли
	Оповестить("ИзменилосьКоличествоСтрокиОтходы",ПарметрыОповещения);
	Закрыть();
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ВидОперации = УГМК_МобильныйСерверВызовСервера.ПолучитьОписьМеталла() Тогда
		Элементы.ГруппаОписьМеталлаКоличество.Видимость = Ложь;
		Элементы.КнопкаВзяли.Заголовок = "Нетто";
...
HTTP = Новый HTTPСоединение("10.72.3.115",44501,"update1c","St7658",,,,);
//создадим структуру 
ЗапросHTTP = Новый HTTPЗапрос("copyOCM_OU/hs/DataOU/wtara/6e62d245-122a-11e4-93fa-40f2e922914b");
...
//запрос
//TODO: добавить запрос к текущим данным ИБ
